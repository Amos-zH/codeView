...
* update by heqing0712
*/

require("./skins/facebook.css");
...
var _ie6 = window.ActiveXObject && !window.XMLHttpRequest,
    ...
    */
    ...
    /*! 取皮肤样式名，默认为 default */
    _skin = _getArgs('skin') || window.dialogSkin || 'default',
...
//            path = window.dialogPath || _path;
      
//        link.href = path + 'skins/' + _skin + '.css';
//        link.rel = 'stylesheet';
//        link.id = 'dialoglink';
//        head.insertBefore(link, head.firstChild);
//    }
//})(_doc.getElementById('dialoglink'));
...
var dialog = function (config) {
    ...
    // zIndex全局配置
    dialog.setting.zIndex = config.zIndex;
...
dialog.fn = dialog.prototype =
    {
        ...
        _init: function (config) {
            ...
            var wsize = that._maxSize();
            if (width && width.toString().indexOf("%") !== -1) {
                config.width = wsize.w * (parseInt(width.split("%")[0]) / 100) + "px";

            }
            if (height && height.toString().indexOf("%") !== -1) {
                config.height = wsize.h * (parseInt(height.split("%")[0]) / 100) + "px";
            }
            ...
            if (document.getElementById("dialoglink") && config.skin != "") {
                document.getElementById("dialoglink").setAttribute("href", config.path + "skins/" + config.skin + ".css");
            }
        ...
        */
        ...
        show: function (args) {
            ...
            var showClassName = config.showClassName;

            wrap.style.visibility = 'visible';
            if (config.className) classNames.push(config.className)
            if (config.showClassName) classNames.push(config.showClassName);
            if (classNames.length) wrap.className = classNames.join(' ');
        ...
        hide: function (args) {
            ...
            if (config.className) wrap.className = config.className;
...
dialog.setting = {
    ...
    className: null,            // 最顶层窗体节点className
    showClassName: null         //顶层窗体显示的时候ClassName
};
...
*/
var _use,
    _isSetCapture = 'setCapture' in _root,
...
$(function () {
    ...
    // 增强dialog拖拽体验（可选外置模块，如不需要可删除）
    // 防止鼠标落入iframe导致不流畅，对超大对话框拖动优化
    if (!dialog.setting.extendDrag) return;
